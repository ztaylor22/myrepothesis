...
```{r}
master_df$Date <- as.Date(master_df$Date) #To convert Date column from characters to dates 
master_df$Date<-format(master_df$Date, format="%m-%d") #To remove year from date column
#If the above doesn't work, I could just create a new chr column for visual purposes that is %m-%d and keep date as %m-%d-%y
master_df
```
